...
uniform int NoiseSignalToNoiseRatio < __UNIFORM_SLIDER_INT1
	ui_min = 0; ui_max = 16;
	ui_label = "Noise Signal-to-Noise Ratio";
...
float3 FilmGrain_LGG_Mura(float4 vpos : SV_Position, float2 texcoord : TexCoord) : SV_Target
{
	...
	if (luma >= NoiseBlackCutoff)
	{
		...
		float stn = NoiseSignalToNoiseRatio != 0 ? pow(abs(inv_luma), (float)NoiseSignalToNoiseRatio) : 1.0;
	...
	{
		const float mura_aspect = 1280.0 / 800.0;
		float screen_aspect = ReShade::ScreenSize.x / ReShade::ScreenSize.y;
		float2 mura_uv = texcoord;

		if (screen_aspect > mura_aspect)
		{
			float scale = mura_aspect / screen_aspect;
			mura_uv.y = (mura_uv.y - 0.5) * scale + 0.5;
		}
		else
		{
			float scale = screen_aspect / mura_aspect;
